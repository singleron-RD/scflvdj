//
// Subworkflow with functionality specific to the singleron-RD/scflvdj pipeline
//
...
workflow PIPELINE_INITIALISATION {
    ...
    Channel
        ...
        .map {
            meta, fastq_1, fastq_2, match_barcode ->
                    return [ meta.id, meta + [ single_end:false ], [ fastq_1, fastq_2 ], match_barcode ]
        }
        .groupTuple()
        .map {
        ...
        .map {
            meta, fastqs, match_barcode ->
                return [ meta, fastqs.flatten(), match_barcode ]
        }
        .set { ch_samplesheet }
...
def validateInputSamplesheet(input) {
    def (metas, fastqs, match_barcode) = input[1..3]
    ...
    return [ metas[0], fastqs, match_barcode[0]]
}
